...
def generate_section(ch_id, sec_id, title, choice_templates, fill_templates):
    lines = [
        f"# 微積分題庫 - 5.{sec_id} 節：{title}",
        "",
        f"本節收錄 5.{sec_id} 節相關題目。",
        ""
    ]
...
for i in range(1, 21):
    ...
    if i % 2 == 0:
        ...
        sol = f"面積為 $\\int_0^1 ((x + {a}) - (x^2 + {a})) dx = \\int_0^1 (x - x^2) dx = [\\frac{{1}}{{2}}x^2 - \\frac{{1}}{{3}}x^3]_0^1 = \\frac{{1}}{{2}} - \\frac{{1}}{{3}} = \\frac{{1}}{{6}}$。"
    else:
        text = f"求曲線 $y = {a}x$ 與 $y = x^2$ 在第一象限所圍成區域的面積。"
        ans = "C" if i % 4 == 1 else "D"
        opts = [f"$\\frac{{{a*a*a}}}{{6}}$", f"$\\frac{{{a}}}{{3}}$", f"$\\frac{{{a*a}}}{{2}}$", f"$1$"]
        sol = f"求交點：${a}x = x^2 \\implies x=0, {a}$。面積為 $\\int_0^{a} ({a}x - x^2) dx = [\\frac{{{a}}}{{2}}x^2 - \\frac{{1}}{{3}}x^3]_0^{a} = \\frac{{{a*a*a}}}{{2}} - \\frac{{{a*a*a}}}{{3}} = \\frac{{{a*a*a}}}{{6}}$。"
...
for i in range(1, 21):
    ...
    if i % 2 == 0:
        ...
        sol = f"圓盤法：$V = \\int_0^1 \\pi (y)^2 dx = \\int_0^1 \\pi ({a}\\sqrt{{x}})^2 dx = \\pi {a*a} \\int_0^1 x dx = \\pi {a*a} [\\frac{{1}}{{2}}x^2]_0^1 = \\frac{{{a*a}\\pi}}{{2}}$。"
    else:
        text = f"求區域由 $y = {a}x$、 $x=1$、與 $y=0$ 所圍成，繞 $x$ 軸旋轉所得之體積為何？"
        ans = "C" if i % 4 == 1 else "D"
        opts = [f"$\\frac{{{a*a}\\pi}}{{3}}$", f"$\\frac{{{a*a}\\pi}}{{2}}$", f"${a*a}\\pi$", f"$3\\pi$"]
        sol = f"體積 $V = \\pi \\int_0^1 ({a}x)^2 dx = \\pi {a*a} [\\frac{{1}}{{3}}x^3]_0^1 = \\frac{{{a*a}\\pi}}{{3}}$。"
...
for i in range(21, 31):
    ...
    sol = f"體積除以 $\\pi$ 為：$\\int_0^2 ({c}x)^2 dx = {c*c} \\int_0^2 x^2 dx = {c*c} [\\frac{{x^3}}{{3}}]_0^2 = {c*c} \\times \\frac{{8}}{{3}} = \\frac{{{c*c*8}}}{{3}}$。計算結果為 {round((c*c*8)/3, 2)}。"
...
for i in range(1, 21):
    ...
    if i % 2 == 0:
        ...
        sol = f"圓柱殼層法：$V = \\int_0^1 2\\pi x ({a}x^2) dx = 2\\pi {a} \\int_0^1 x^3 dx = 2\\pi {a} [\\frac{{1}}{{4}}x^4]_0^1 = \\frac{{{a}\\pi}}{{2}}$。"
    else:
        text = f"求 $y = {a}x$、 $y=0$、 $x=2$ 繞 $y$ 軸旋轉之旋轉體體積為何？"
        ans = "A" if i % 4 == 1 else "B"
        opts = [f"$\\frac{{{a*16}\\pi}}{{3}}$", f"$\\frac{{{a*8}\\pi}}{{3}}$", f"$\\frac{{{a}\\pi}}{{3}}$", f"$16\\pi$"]
        sol = f"圓柱殼層法：$V = \\int_0^2 2\\pi x ({a}x) dx = 2\\pi {a} \\int_0^2 x^2 dx = 2\\pi {a} [\\frac{{x^3}}{{3}}]_0^2 = \\frac{{{a*16}\\pi}}{{3}}$。"
...
for i in range(21, 31):
    ...
    sol = f"體積為 $\\int_0^2 2\\pi x ({c}x^2) dx = 2\\pi {c} [\\frac{{x^4}}{{4}}]_0^2 = 2\\pi {c} \\times 4 = 8{c}\\pi$。除以 $\\pi$ 為 $8c = {c*8}$。"
    ans_val = c * 8
...
for i in range(1, 21):
    a = i + 2
    if i % 2 == 0:
        text = f"一彈簧在拉長 $x$ 公尺時所需的力為 $F(x) = {a}x$ 牛頓，試問將其從 $x=0$ 拉長至 $x=2$ 公尺所需的功（Work）為多少焦耳？"
        ans = "B" if i % 4 == 0 else "A"
        opts = [f"${a*2}$", f"${a*4}$", f"${a}$", f"$0$"]
        sol = f"功即為力的定積分：$W = \\int_0^2 {a}x dx = [\\frac{{{a}}}{{2}}x^2]_0^2 = \\frac{{{a}}}{{2}} \\times 4 = {a*2}$。"
    else:
        text = f"若將一條拉力函數為 $F(x) = {a}x^2$ 的非線性彈簧從 $x=0$ 拉長至 $x=3$，則所作之功為何？"
        ans = "C" if i % 4 == 1 else "D"
        opts = [f"${a*9}$", f"${a*3}$", f"${a*27}$", f"$9$"]
        sol = f"功 $W = \\int_0^3 {a}x^2 dx = [\\frac{{{a}}}{{3}}x^3]_0^3 = \\frac{{{a}}}{{3}} \\times 27 = {a*9}$。"
...
for i in range(21, 31):
    ...
    sol = f"功為 $\\int_0^4 {c}x dx = [\\frac{{{c}}}{{2}}x^2]_0^4 = \\frac{{{c}}}{{2}} \\times 16 = {c*8}$。"
    ans_val = c * 8
...
for i in range(1, 21):
    a = i + 1
    if i % 2 == 0:
        text = f"求函數 $f(x) = {a}x^2$ 在區間 $[0, 3]$ 的平均值（Average Value）。"
        ans = "A" if i % 4 == 0 else "B"
        opts = [f"${a*3}$", f"${a*9}$", f"${a}$", f"$\\frac{{{a}}}{{3}}$"]
        sol = f"平均值 $f_{{avg}} = \\frac{{1}}{{3-0}} \\int_0^3 {a}x^2 dx = \\frac{{1}}{{3}} [\\frac{{{a}}}{{3}}x^3]_0^3 = \\frac{{1}}{{3}} \\times \\frac{{{a}}}{{3}} \\times 27 = {a*3}$。"
    else:
        text = f"求函數 $f(x) = {a}x$ 在區間 $[1, 5]$ 的平均值為何？"
        ans = "C" if i % 4 == 1 else "D"
        opts = [f"${a*3}$", f"${a*2}$", f"${a*4}$", f"${a}$"]
        sol = f"平均值 $f_{{avg}} = \\frac{{1}}{{5-1}} \\int_1^5 {a}x dx = \\frac{{1}}{{4}} [\\frac{{{a}}}{{2}}x^2]_1^5 = \\frac{{1}}{{4}} \\times \\frac{{{a}}}{{2}}(25 - 1) = \\frac{{24{a}}}{{8}} = {a*3}$。"
...
for i in range(21, 31):
    ...
    sol = f"平均值為 $\\frac{{1}}{{6-0}} \\int_0^6 {c}x dx = \\frac{{1}}{{6}} [\\frac{{{c}}}{{2}}x^2]_0^6 = \\frac{{1}}{{6}} \\times \\frac{{{c}}}{{2}} \\times 36 = {c*3}$。"
    ans_val = c * 3
...
if __name__ == "__main__":
    ...
    print("Chapter 5 sections generated successfully.")
